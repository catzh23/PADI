...
public class ScheduleC extends Schedule {
  ...
  public List<Integer> acceptors(int ballot) {
    if (ballot < 2) return this.acceptors_start;
    else return this.acceptors_end;
  }

  public List<Integer> acceptorsinrange(int low_ballot, int high_ballot) {
    if (high_ballot < 2) return this.acceptors_start;
    else if (low_ballot >= 2) return this.acceptors_end;
    else return this.acceptors_all;
  }
  ...
  public boolean isacceptor(int node, int ballot) {
    if (ballot < 2) return this.acceptors_start.contains(node);
    else return this.acceptors_end.contains(node);
  }

  public Integer leader(int ballot) {
    if (ballot < 2) return new Integer(ballot);
    else if (ballot < 5) return new Integer(ballot + 1);
    else return new Integer(5);
  }

  public Integer quorum(int ballot) {
    if (ballot < 2) return new Integer(2);
