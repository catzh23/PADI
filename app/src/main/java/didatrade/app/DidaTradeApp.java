...
public class DidaTradeApp {
  ...
  private boolean populate(int quantity) {
    ...
    DidaTradeMain.PopulateRequest.Builder populate_request =
        DidaTradeMain.PopulateRequest.newBuilder();
    populate_request.setReqid(reqid);
    populate_request.setQuantity(quantity);

    ArrayList<DidaTradeMain.PopulateReply> populate_responses =
        new ArrayList<DidaTradeMain.PopulateReply>();
    GenericResponseCollector<DidaTradeMain.PopulateReply> populate_collector =
        new GenericResponseCollector<DidaTradeMain.PopulateReply>(
            populate_responses, this.n_servers);

    for (int i = 0; i < n_servers; i++) {
      CollectorStreamObserver<DidaTradeMain.PopulateReply> populate_observer =
          new CollectorStreamObserver<DidaTradeMain.PopulateReply>(populate_collector);
    ...
    } else System.out.println("Panic...error executing populate\n");
  ...
  private boolean adduser(int uid, int cash, int stock) {
    ...
    DidaTradeMain.AddUserRequest.Builder adduser_request =
        DidaTradeMain.AddUserRequest.newBuilder();
    adduser_request.setReqid(reqid);
    adduser_request.setUserid(uid);
    adduser_request.setCash(cash);
    adduser_request.setStock(stock);

    ArrayList<DidaTradeMain.AddUserReply> adduser_responses =
        new ArrayList<DidaTradeMain.AddUserReply>();
    GenericResponseCollector<DidaTradeMain.AddUserReply> adduser_collector =
        new GenericResponseCollector<DidaTradeMain.AddUserReply>(adduser_responses, this.n_servers);

    for (int i = 0; i < n_servers; i++) {
      CollectorStreamObserver<DidaTradeMain.AddUserReply> adduser_observer =
          new CollectorStreamObserver<DidaTradeMain.AddUserReply>(adduser_collector);
    ...
    if (adduser_responses.size() >= responses_needed) {
      ...
      if (result) {
        System.out.println(
            "User " + uid + " added with cash " + cash + "and stock" + stock + "% of stock\n");
      } else {
        System.out.println("Adduser operation failed\n");
      }
    } else System.out.println("Panic...error in sell operation\n");
  ...
  private boolean sell(int uid, int quantity) {
    ...
    GenericResponseCollector<DidaTradeMain.SellReply> sell_collector =
        new GenericResponseCollector<DidaTradeMain.SellReply>(sell_responses, this.n_servers);

    for (int i = 0; i < n_servers; i++) {
      CollectorStreamObserver<DidaTradeMain.SellReply> sell_observer =
          new CollectorStreamObserver<DidaTradeMain.SellReply>(sell_collector);
  ...
  private boolean buy(int uid, int quantity) {
    ...
    GenericResponseCollector<DidaTradeMain.BuyReply> buy_collector =
        new GenericResponseCollector<DidaTradeMain.BuyReply>(buy_responses, this.n_servers);

    for (int i = 0; i < n_servers; i++) {
      CollectorStreamObserver<DidaTradeMain.BuyReply> buy_observer =
          new CollectorStreamObserver<DidaTradeMain.BuyReply>(buy_collector);
    ...
    } else System.out.println("Panic...error in buy operation\n");
  ...
  private int balance(int uid) {
    ...
    DidaTradeMain.BalanceRequest.Builder balance_request =
        DidaTradeMain.BalanceRequest.newBuilder();
    balance_request.setReqid(reqid);
    balance_request.setUserid(uid);

    ArrayList<DidaTradeMain.BalanceReply> balance_responses =
        new ArrayList<DidaTradeMain.BalanceReply>();
    GenericResponseCollector<DidaTradeMain.BalanceReply> balance_collector =
        new GenericResponseCollector<DidaTradeMain.BalanceReply>(balance_responses, this.n_servers);

    for (int i = 0; i < n_servers; i++) {
      CollectorStreamObserver<DidaTradeMain.BalanceReply> balance_observer =
          new CollectorStreamObserver<DidaTradeMain.BalanceReply>(balance_collector);
    ...
    } else System.out.println("Panic...error in balance operation\n");
  ...
  private boolean show() {
    ...
    GenericResponseCollector<DidaTradeMain.DumpReply> dump_collector =
        new GenericResponseCollector<DidaTradeMain.DumpReply>(dump_responses, n_servers);

    for (int i = 0; i < n_servers; i++) {
      CollectorStreamObserver<DidaTradeMain.DumpReply> dump_observer =
          new CollectorStreamObserver<DidaTradeMain.DumpReply>(dump_collector);
    ...
    } else System.out.println("Panic...error on dump request\n");
  ...
  private void doStuff(int task) {
    ...
    while (counter < this.loop_size) {
      if (task == -1) next_action = rnd.nextInt(2);

      int uid = rnd.nextInt(this.user_range);
      int quantity = rnd.nextInt(100);
      if (next_action == 0) this.sell(uid, quantity);
      else this.buy(uid, quantity);
      counter++;
    }
  }
    ...
    while (cursor < length) {
      ...
      switch (option_name) {
        ...
        case "--lenght":
          ...
          else this.loop_size = Integer.parseInt(option_parameter);
          break;
        case "--sleep":
          if (option_parameter == null) System.err.println("missing sleeprange");
          else this.sleep_range = Integer.parseInt(option_parameter);
          break;
        case "-i":
          this.interactive_mode = true;
          break;
        ...
        case "populate":
          ...
          if (parameter1 != null) {
            try {
              ...
              } else System.out.println("failed to populate.");
            } catch (NumberFormatException e) {
              System.out.println("usage: populate n_users");
            }
          } else System.out.println("usage: populate n_users");
          break;
        case "add":
          ...
          if (parameter1 != null && parameter2 != null && parameter3 != null) {
            try {
              ...
              if (this.adduser(uid, cash, stock)) System.out.println("user " + uid + "created.");
              else System.out.println("user " + uid + " create failed.");
            } catch (NumberFormatException e) {
              System.out.println("usage: add uid cash stocl");
            }
          } else System.out.println("usage: add uid cash stock");
          break;
        case "sell":
          ...
          if (parameter1 != null && parameter2 != null) {
            try {
              ...
              else System.out.println("user " + uid + " sell operation failed.");
            } catch (NumberFormatException e) {
              System.out.println("usage: sell uid quantity");
            }
          } else System.out.println("usage: sell uid quantity");
          break;
        case "buy":
          ...
          if (parameter1 != null && parameter2 != null) {
            try {
              ...
              else System.out.println("user " + uid + " buy operation failed.");
            } catch (NumberFormatException e) {
              System.out.println("usage: buy uid quantity");
            }
          } else System.out.println("usage: buy uid quantity");
          break;
        case "balance":
          ...
          if (parameter1 != null) {
            try {
              ...
              if (balance != -1)
                System.out.println("user " + uid + " balance is  " + balance + ".");
              else System.out.println("user " + uid + " balanceoperation failed.");
            } catch (NumberFormatException e) {
              System.out.println("usage: balance uid");
            }
          } else System.out.println("usage: balance uid");
          break;
        case "show":
          try {
            this.show();
  ...
  public void main_loop(String[] args) throws Exception {
    ...
    if (args.length < 4) {
      ...
      System.err.printf(
          "Usage: java %s client id host port schedule%n", DidaTradeApp.class.getName());
      return;
    }
    ...
    // print parameters
    System.out.println(
        "Client id = " + this.client_id + " serverhost = " + this.host + " port = " + this.port);
    ...
    // do work
    if (interactive_mode == false) this.doStuff(-1);
    else this.goInteractive();
