...
public class TradeManager {
  ...
  public synchronized boolean populate(int quantity) {
    for (int i = 0; i < quantity; i++) this.add_user(i, quantity, quantity);
  ...
  private void match_all() {
    while ((this.sales_orders.size() > 0) && (this.buy_orders.size() > 0)) {
      ...
      if (sale_order.available() == 0) this.sales_orders.remove(0);
      if (buy_order.pending() == 0) this.buy_orders.remove(0);
    }
  }
