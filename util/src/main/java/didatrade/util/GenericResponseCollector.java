...
public class GenericResponseCollector<T> {
  ...
  public GenericResponseCollector(
      ArrayList<T> responses, int maxresponses, GenericResponseProcessor p) {
    this.processor = p;
    this.collected_responses = responses;
    this.received = 0;
    this.pending = maxresponses;
  ...
  public synchronized void addResponse(T resp) {
    if (!this.done) {
      ...
      if (this.processor != null) this.done = this.processor.onNext(this.collected_responses, resp);
    }
    this.received++;
    this.pending--;
    if (this.pending == 0) this.done = true;
    notifyAll();
  }
