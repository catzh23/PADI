...
public class Console {
  ...
  public synchronized void setBallotCompleted(int ballot) {
    if (ballot > this.ballot_completed) this.ballot_completed = ballot;
  }
  ...
  public synchronized void setLastBallot(int ballot) {
    if (ballot > this.last_ballot) this.last_ballot = ballot;
  }
  ...
  public void main_loop(String[] args) throws Exception {
    ...
    DidaTradeMasterServiceGrpc.DidaTradeMasterServiceStub[] console_async_stubs =
        new DidaTradeMasterServiceGrpc.DidaTradeMasterServiceStub[n_servers];
        ...
        case "ballot":
          ...
          if ((parameter1 != null) && (parameter2 != null)) {
            ...
            if (ballot_number < this.getLastBallot())
              System.out.println(
                  "usage: ballot must be larger or equal than " + last_ballot + ".\n");
            else {
              ...
              try {
                ...
                System.out.println(
                    "sending ballot " + ballot_number + " to replica " + replica + ".\n");

                sequence_number = sequence_number + 1;
                int reqid = sequence_number * 100 + client_id;

                DidaTradeMaster.NewBallotRequest.Builder newballot_request =
                    DidaTradeMaster.NewBallotRequest.newBuilder();
                ArrayList<DidaTradeMaster.NewBallotReply> newballot_responses =
                    new ArrayList<DidaTradeMaster.NewBallotReply>();
                ;
                GenericResponseCollector<DidaTradeMaster.NewBallotReply> newballot_collector =
                    new GenericResponseCollector<DidaTradeMaster.NewBallotReply>(
                        newballot_responses, 1);
                CollectorStreamObserver<DidaTradeMaster.NewBallotReply> newballot_observer =
                    new CollectorStreamObserver<DidaTradeMaster.NewBallotReply>(
                        newballot_collector);
                ...
                console_async_stubs[replica].newballot(
                    newballot_request.build(), newballot_observer);
                        ...
                        new Runnable() {
                          public void run() {
                            ...
                            if (newballot_responses.size() >= 1) {
                              Iterator<DidaTradeMaster.NewBallotReply> newballot_iterator =
                                  newballot_responses.iterator();
                              DidaTradeMaster.NewBallotReply newballot_reply =
                                  newballot_iterator.next();
                              int completed = newballot_reply.getCompletedballot();
                              System.out.println(
                                  "reply received to new ballot request for ballot = "
                                      + ballot_number
                                      + " with completed = "
                                      + completed);
                              ...
                              System.out.println(
                                  "no reply received to new ballot request for ballot = "
                                      + ballot_number);
                          }
                        })
                    .start();
          ...
          } else System.out.println("usage: ballot number replica");
          break;
        case "activate":
          ...
          if (parameter1 != null) {
            ...
            if (ballot_number < this.getLastBallot())
              ...
              try {
                ...
                System.out.println(
                    "sending activate " + ballot_number + " to replica " + replica + ".\n");

                sequence_number = sequence_number + 1;
                int reqid = sequence_number * 100 + client_id;

                DidaTradeMaster.ActivateRequest.Builder activate_request =
                    DidaTradeMaster.ActivateRequest.newBuilder();
                ArrayList<DidaTradeMaster.ActivateReply> activate_responses =
                    new ArrayList<DidaTradeMaster.ActivateReply>();
                ;
                GenericResponseCollector<DidaTradeMaster.ActivateReply> activate_collector =
                    new GenericResponseCollector<DidaTradeMaster.ActivateReply>(
                        activate_responses, 1);
                CollectorStreamObserver<DidaTradeMaster.ActivateReply> activate_observer =
                    new CollectorStreamObserver<DidaTradeMaster.ActivateReply>(activate_collector);
                        ...
                        new Runnable() {
                          public void run() {
                            ...
                            if (activate_responses.size() >= 1) {
                              Iterator<DidaTradeMaster.ActivateReply> activate_iterator =
                                  activate_responses.iterator();
                              DidaTradeMaster.ActivateReply activate_reply =
                                  activate_iterator.next();
                              System.out.println(
                                  "reply received to activate request for ballot = "
                                      + ballot_number
                                      + " with ack = "
                                      + activate_reply.getAck());
                            } else
                              System.out.println(
                                  "no reply received to activate request for ballot = "
                                      + ballot_number);
                          }
                        })
                    .start();
          ...
          } else System.out.println("usage: activate replica");
          break;
        case "debug":
          ...
          if ((parameter1 != null) && (parameter2 != null)) {
            try {
              ...
              DidaTradeMaster.SetDebugRequest.Builder setdebug_request =
                  DidaTradeMaster.SetDebugRequest.newBuilder();
              ArrayList<DidaTradeMaster.SetDebugReply> setdebug_responses =
                  new ArrayList<DidaTradeMaster.SetDebugReply>();
              ;
              GenericResponseCollector<DidaTradeMaster.SetDebugReply> setdebug_collector =
                  new GenericResponseCollector<DidaTradeMaster.SetDebugReply>(
                      setdebug_responses, 1);
              CollectorStreamObserver<DidaTradeMaster.SetDebugReply> setdebug_observer =
                  new CollectorStreamObserver<DidaTradeMaster.SetDebugReply>(setdebug_collector);
              ...
              if (setdebug_responses.size() >= 1) {
                Iterator<DidaTradeMaster.SetDebugReply> setdebug_iterator =
